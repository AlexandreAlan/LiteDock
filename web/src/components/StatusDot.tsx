...
const MAP: Record<string, { dot: string; ring: string; label: string; pulse?: boolean }> = {
  running: { dot: 'bg-ok', ring: 'shadow-[0_0_0_3px_rgba(127,166,80,0.18)]', label: 'no ar', pulse: true },
  online: { dot: 'bg-ok', ring: 'shadow-[0_0_0_3px_rgba(127,166,80,0.18)]', label: 'no ar', pulse: true },
...
export function StatusDot({ state, withLabel = false }: { state?: string | null; withLabel?: boolean }) {
  ...
  // online/running ganham anel pulsante (sinal "vivo"); demais ficam estáticos.
  const pulse = s.pulse ? 'relative status-pulse' : '';
  return (
    <span className="inline-flex items-center gap-2">
      <span className={`inline-block h-2.5 w-2.5 rounded-full ${s.dot} ${s.ring} ${pulse}`} />
